...
for k = 1:length(Nq)
    ...
    % Par block de trois points
    for i = 1:2:(length(xi)-2)
        % Identifier les points du domaine dans [xi(i), xi(i+2)]
        idx_block = (domain >= xi(i)) & (domain <= xi(i+2));
        dom_block = domain(idx_block);
        
        % Construire le polynôme de degré 2 via polynome_newton
        [~, ~, ~, utile_block] = polynome_newton(xi(i:i+2), fi(i:i+2), dom_block);
        
        % Insérer dans Q_n
        Qn(idx_block) = utile_block;
    end
    % Calculer l'erreur maximale E(h) = max |f - Q_n|
    Ex = abs(Qn - fedex);
    Eq(k) = max(Ex);
end
...
loglog(href, Cref * href.^3, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1);
